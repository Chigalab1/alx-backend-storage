...
def call_history(method: Callable) -> Callable:
    ...
    def wrapper(self: Any, *args) -> str:
        """ Func that Wraps called method and tracks its passed argument by storing
            them to redis
        """
...
class Cache:
    ...
    def __init__(self) -> None:
        """ Func that Initialize new cache object
        """
    ...
    def store(self, data: Union[str, bytes,  int,  float]) -> str:
        """ Func that Stores data in redis with randomly generated key
        """
        key = str(uuid4())
        client = self._redis
        client.set(key, data)
    ...
    def get(self, key: str, fn: Optional[Callable] = None) -> Any:
        """Func that  Gets key's value from redis and converts
    ...
    def get_str(self, data: bytes) -> str:
        """ Func that Converts bytes to string
        """
        return data.decode('utf-8')

    def get_int(self, data: bytes) -> int:
        """ Func that Converts bytes to integers
        """
        return int(data)
